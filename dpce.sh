...
collect_data_export () {
    read -epr "Drush alias? (optional - e.x., @alias): " d_alias
    test_root_dir
    read -epr "Directory to export config (this should be outside of your Drupal repository): " export_dir_in    
    ...
    if [ -d "$export_dir" ]; then        
        ...
        read -epr "Directory $export_dir does not exist. Create? [y/n]:" create_export_dir
...
find_configs () {
    read -epr "Find config files containing [glob]?: " glob
    read -epr "Exclude files containing [glob]?: " notglob
...
confirm () {
    read -epr "Confirm list? [y/n]: " isconfirmed
...
import () {
    read -epr "Import directory? (This should be inside your Drupal repository but outside your docroot): " import_dir
    realimportdir=$(readlink -f "$import_dir")

    if [ -d "$import_dir" ]; then        
        cd "$export_dir" && find . -name "$glob" ! -name "$notglob" -0 | xargs cp -t "$realimportdir"
        ...
        read -epr "Directory \"$import_dir\" does not exist. Create it? [y/n]: " create_confirm
